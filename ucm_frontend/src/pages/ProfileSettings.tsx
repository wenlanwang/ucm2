...
export default function ProfileSettings() {
  const { user, ssoStatus } = useAuthStore();
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState('info');
  const [passwordForm] = Form.useForm();

  // 判断是否是生产环境SSO登录
  const isProductionSSO = ssoStatus && !ssoStatus.use_mock;
  ...
  // 根据是否是生产SSO登录，决定是否显示修改密码Tab
  const tabItems = isProductionSSO ? [infoTab] : [infoTab, passwordTab];
